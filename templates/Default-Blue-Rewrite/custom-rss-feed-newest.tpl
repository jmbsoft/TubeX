...
{foreach var=$video from=$videos}
    <item>
      ...
      <link>{$g_config.base_url}/video/{$video.video_id}/{$video.title|t_urlify(7)}</link>
      <description>
      <![CDATA[
      <a href="{$g_config.base_url}/video/{$video.video_id}/{$video.title|t_urlify(7)}" target="_blank">
