...
{template file="global-header.tpl" title=$title}
    ...
    <div class="main-content page-content">
      ...
      <div class="section-content-no-header text-center fs130">
        <a {if !$g_loc_videos_popular}href="{$g_config.base_uri}/videos/popular/"{/if}>{"_Text:Popular"}</a> &mdash;
        <a {if !$g_loc_videos_top_rated}href="{$g_config.base_uri}/videos/top-rated/"{/if}>{"_Text:Top Rated"}</a> &mdash;
        <a {if !$g_loc_videos_most_rated}href="{$g_config.base_uri}/videos/most-rated/"{/if}>{"_Text:Most Rated"}</a> &mdash;
        <a {if !$g_loc_videos_most_discussed}href="{$g_config.base_uri}/videos/most-discussed/"{/if}>{"_Text:Most Discussed"}</a> &mdash;
        <a {if !$g_loc_videos_top_favorited}href="{$g_config.base_uri}/videos/top-favorited/"{/if}>{"_Text:Top Favorited"}</a>
      </div>
      ...
      <span class="section-left">
        ...
        <div class="section-content section-videos">
          ...
          <div style="position: absolute; right: 10px; top: 6px;" class="fs80">
            <a {if !$g_loc_today}href="{$g_config.base_uri}/videos/{$g_sort}/today/"{/if}>{"_Text:Today"}</a> &mdash;
            <a {if !$g_loc_week}href="{$g_config.base_uri}/videos/{$g_sort}/week/"{/if}>{"_Text:This Week"}</a> &mdash;
            <a {if !$g_loc_month}href="{$g_config.base_uri}/videos/{$g_sort}/month/"{/if}>{"_Text:This Month"}</a> &mdash;
            <a {if !$g_loc_all_time}href="{$g_config.base_uri}/videos/{$g_sort}/all-time/"{/if}>{"_Text:All Time"}</a>
          </div>
